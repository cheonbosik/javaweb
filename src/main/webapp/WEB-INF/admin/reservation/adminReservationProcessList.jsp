...
<div class="container">
  <h2 class="text-center">
    <c:if test="${empty mid}">전체 예약 처리된 리스트</c:if>
    <c:if test="${!empty mid}">아이디 '${mid}'(으)로 예약 처리된 리스트</c:if>
  </h2>
  <br/>
  <table class="table table-borderless m-0">
    <tr>
      <td><a href="${ctp}/AdminReservationProcessList.res" class="btn btn-primary">전체 예약 List보기</a></td>
      <td class="text-right">
        <form name="idSearchForm" method="post" action="${ctp}/AdminReservationProcessList.res">
	        <input type="text" name="mid" id="mid" placeholder="검색할 아이디 입력"/>
	        <input type="button" value="아이디검색" onclick="midCheck()" class="btn btn-success"/>
        </form>
      </td>
    </tr>
  </table>
