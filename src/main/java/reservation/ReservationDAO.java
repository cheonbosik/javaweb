...
public class ReservationDAO {
  ...
  // 개별회원이 예약한 모든내역들 가져오기.
	public ArrayList<ReservationVO> getReservationMidList(int startIndexNo, int pageSize, String mid, String reservationSw) {
			...
			if(mid.equals("")) {
				...
				if(reservationSw.equals("admin")) {
					sql = "select *, "
							...
							+ "(select content from reservationInformation where idx = r.resInforIdx) as content, "
							+ "(select photo from reservationInformation where idx = r.resInforIdx) as photo "
							+ " from reservation r where r.mid like ? order by r.idx desc limit ?,?";
					pstmt = conn.prepareStatement(sql);
					pstmt.setString(1, "%"+mid+"%");
